...
- Планировщик — **отдельный сервис** (`scheduler`). Если он не запущен, попытка
  запланировать задачу из чата вернёт **ошибку** (задача не создаётся).
...
- **Один сервис + heartbeat.** Планировщик ровно один (сервис `scheduler`); он
  обновляет heartbeat в SQLite. Инструмент `schedule` проверяет heartbeat и при
  отсутствии живого планировщика **возвращает ошибку, не создавая задачу** —
  чтобы не было «тихих» задач, которые никогда не выполнятся. Heartbeat
  owner-scoped, поэтому ещё и защищает от случайного запуска двух планировщиков
  (задачи выполняет только держатель).
