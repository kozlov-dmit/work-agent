...
- **Доставка**: Telegram-сообщение (если задан чат и доступен бот) или файл в
  `schedule-output/<id>/`. 
- **Запуск**: планировщик — отдельный сервис `work-agent scheduler` (в `docker
  compose up`). На каждом тике обновляет heartbeat (`Lease` в SQLite
  `scheduler.db`). Инструмент `schedule` проверяет `scheduler_alive` и при
  отсутствии живого планировщика **возвращает ошибку и не создаёт задачу**.
  Heartbeat owner-scoped — заодно защищает от случайного запуска двух
  планировщиков (задачи выполняет только держатель). Задачи из любого фронтенда
  подхватываются через общий стор.
...
- **Web** (`frontends/web.py`): FastAPI. Чат — одностраничный UI; каждое
  ...
  - *Метрики* — расход токенов, надёжность LLM, счётчики инструментов
    (`metrics.py`, `MetricsStore` поверх SQLite в общем volume — **агрегат со
    всех процессов**: web/telegram/scheduler). Системные метрики (CPU/RAM/сеть,
    `psutil`) собираются с каждого сервиса (демон-поток `start_system_reporter`
    пишет свой замер в таблицу `system_samples`) и показываются **в разрезе по
    сервисам**. Автообновление.
...
│   ├── metrics.py             # общие метрики (SQLite, агрегат со всех процессов)
