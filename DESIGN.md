...
- **Доставка**: Telegram-сообщение (если задан чат и доступен бот) или файл в
  `schedule-output/<id>/`. 
- **Запуск и отказоустойчивость**: цикл планировщика работает во всех
  долгоживущих сервисах (web, telegram, scheduler — `start_background_scheduler`),
  но задачи выполняет только держатель **аренды-лидерства** (`Lease`, атомарный
  лок в SQLite `scheduler.db`) — двойного запуска нет, а при смерти лидера аренду
  перехватывает другой живой сервис. `schedule` предупреждает, если живого
  планировщика нет (`scheduler_alive`). Задачи из любого фронтенда подхватываются
  через общий стор.
