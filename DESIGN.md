...
- **Web** (`frontends/web.py`): FastAPI. Чат — одностраничный UI; каждое
  ...
  - *Метрики* — CPU/RAM/сеть (`psutil`), расход токенов и надёжность LLM
    (`metrics.py`, `MetricsStore` поверх SQLite в общем volume — счётчики
    **агрегируются со всех процессов**: web/telegram/scheduler), счётчики
    инструментов; автообновление. CPU/RAM/сеть — по процессу, отдающему дашборд.
